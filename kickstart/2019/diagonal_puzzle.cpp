...
constexpr const int SLASHED = 1;
constexpr const int BACK_SLASHED = 2;
...
static void solve() {
    ...
    for (int c = 0; c < 4; ++c) {
        ...
        for (int r = 0, c = 0; r < N / 2; ++r, ++c)
            if (copy[r][c] == '.') {
                ++cur;
                for (int y = 2 * r + 1, s = 2 * r, d = 0; y; --y, --s, ++d)
                    turn(copy[s][d]);
            }
        for (int r = N - 1, c = N - 1; r >= N / 2; --r, --c)
            if (copy[r][c] == '.') {
                ++cur;
                for (int y = 2 * (N - r) - 1, s = N - 1, d = N - y; y; --y, --s, ++d)
                    turn(copy[s][d]);
            }
