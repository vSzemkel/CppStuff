...
int main() {
	...
	for (int case_num = 1; case_num <= T; case_num ++) {
		...
		tensor<int, 2> shared_cnt({E, E+1});
		for (int i = 0; i < E; i++) {
			for (int t = 0; t < W; t++) {
				int cnt = weight_cnt[{i, t}];
				for (int k = i; k < E; k++) {
					cnt = std::min(cnt, weight_cnt[{k, t}]);
					shared_cnt[{i, k+1}] += cnt;
				}
			}
		}

		tensor<int64_t, 2> dp({E, E+1});
		for (int i = 0; i < E; i++) {
			dp[{i, i+1}] = shared_cnt[{i, i+1}];
		}
		for (int l = 2; l <= E; l++) {
			for (int i = 0, j = l; j <= E; i++, j++) {
				dp[{i, j}] = int64_t(1e18);
				for (int k = i+1; k <= j-1; k++) {
					dp[{i, j}] = std::min(dp[{i, j}], dp[{i, k}] + dp[{k, j}] - shared_cnt[{i, j}]);
				}
			}
		}

		int64_t ans = dp[{0, E}] * 2;
