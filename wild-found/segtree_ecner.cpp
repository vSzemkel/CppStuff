...
struct range
{
    ...
    {
        const int anc_depth = log_2((a - 1) ^ b);
        const int anc_msk = (1 << anc_depth) - 1;
    ...
    void for_parents_down(F f) const
    {
        ...
        const int dx = __builtin_ctz(x);
        const int dy = __builtin_ctz(y);
    ...
    void for_parents_up(F f) const
    {
        ...
        const int dx = __builtin_ctz(x);
        const int dy = __builtin_ctz(y);
        const int anc_depth = log_2((x - 1) ^ y);
...
int main()
{
    ...
    for (int case_num = 1; case_num <= T; case_num++) {
        ...
        for (int i = 0; i < N; ++i)
            locs[hats[i]].push_back(i);
        for (int i = 0; i < N; ++i)
            locs[hats[i]].push_back(N + i);
        ...
        for (seg_tree::point a(layout.N * 2 - 1); a > 0; a--)
            update_node(a);
