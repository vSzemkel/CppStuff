    ...
    switch (signal) {
        ...
        case SIGINDEXINGSTATUS:
            if (g_indexer_processing) {
                ...
                status_report = current_time.tv_sec - g_last_indexing.tv_sec;
            } else
                status_report = -1;

            sv.sival_int = (status_report << 16) + (g_indexer_id & 0xffff);
            sigqueue(getppid(), SIGINDEXINGSTATUS, sv);
            break;
        case SIGINT:
        case SIGTSTP:
...
static void numf_signals(int sig, siginfo_t* info, __attribute__ ((unused)) void *ucontext)
{
    g_fake_eof = 1;
    const int data = info->si_value.sival_int;
    if (sig == SIGINDEXINGSTATUS) {
        char buf[32];
        short status = (data >> 16) & 0xffff;
        if (status < 0)
            strcpy(buf, "inactive");
        else
            sprintf(buf, "active for %i sec.", status);
        ANSWER("Indexer on %s is %s", g_argv[g_dir_parms_pos + (data & 0xffff)], buf);
    } else { // SIGRESETCACHE
...
int main(int argc, char* argv[])
{
    ...
    sigaction(SIGINDEXINGSTATUS, &act, NULL);

    int command = 0;
    char cmd_history[CLI_HISTORY_LEN][MAX_CONSOLE_LINE + 1];
    ...
    while (1) {
        ...
        refresh();
        int pos = 0;
        const int dest_cmd = (command + 1) % CLI_HISTORY_LEN;
        command = dest_cmd;
        char* buf = cmd_history[command];
        while (pos < MAX_CONSOLE_LINE) {
            int cur_x, cur_y, org_x = getcurx(win);
            const int chr = getch();
            if (chr == EOF && errno == EINTR) {
                g_fake_eof = 0;
                continue;
            }
            if (chr == KEY_UP || chr == KEY_DOWN) {
                getyx(win, cur_y, cur_x);
                while (cur_x > org_x - pos)
                    mvdelch(cur_y, --cur_x);
                const int off = chr == KEY_DOWN ? 1 : -1;
                for (int k = 0; k < CLI_HISTORY_LEN; ++k) {
                    command = (CLI_HISTORY_LEN + command + off) % CLI_HISTORY_LEN;
                    if (command != dest_cmd && cmd_history[command][0] != '\0') break;
                }
                if (dest_cmd != command)
                    memcpy(buf, cmd_history[command], MAX_CONSOLE_LINE + 1);
                pos = strlen(buf);
                printw(buf);
                continue;
                ...
                while (cur_x > org_x)
                    mvdelch(cur_y, --cur_x);
                continue;
            } else if (chr == EOF || chr == 4) // EOF, CTRL_D
                goto bye;
...
Compile:
clear && gcc -Wall -Wextra -ggdb3 -Og -fsanitize=address -pthread numf_ncur.c -o numf_ncur -lbsd -lm -lncurses
gcc -Wall -Wextra -O3 -pthread numf_ncur.c -o numf_ncur -lbsd -lm -lncurses

Test run:
./numf_ncur -r -m 301 -M 981 -i 240 /tmp/numf_dir1 /tmp/numf_dir2
...
- using command history buffer - THIS IS A REASON THIS CODE IS HERE
