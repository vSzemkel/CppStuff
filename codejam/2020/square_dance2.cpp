...
int g_rows, g_cols;
std::vector<std::vector<dancer2_t>> g_board2;

void quallify2(dancer2_t& d)
{
    int sum{0};
    if (d.N > 0) sum += d.skill - d.N;
    if (d.S > 0) sum += d.skill - d.S;
    if (d.E > 0) sum += d.skill - d.E;
    if (d.W > 0) sum += d.skill - d.W;

    d.well_dancing = sum >= 0;
}

void quallify3(dancer2_t& d)
{
    int sum{0}, cnt{0};
...
int64_t solve2() {
    ...
    g_board2.assign(g_rows, std::vector<dancer2_t>(g_cols));
    // read input
    for (int r = 0; r < g_rows; ++r)
        for (int c = 0; c < g_cols; ++c)
            std::cin >> g_board2[r][c].skill;
    // initialize
    ...
    for (int r = 0; r < g_rows; ++r)
        for (int c = 0; c < g_cols; ++c) {
            auto& d = g_board2[r][c];
            if (c > 0) {
                d.W = g_board2[r][c - 1].skill;
                d.dist_W = 1;
            }
            if (c < g_cols - 1) {
                d.E = g_board2[r][c + 1].skill;
                d.dist_E = 1;
            }
            if (r > 0) {
                d.N = g_board2[r - 1][c].skill;
                d.dist_N = 1;
            }
            if (r < g_rows - 1) {
                d.S = g_board2[r + 1][c].skill;
                d.dist_S = 1;
            }
            quallify2(d);
    ...
    while (true) {
        ...
        const auto victims = std::partition(still_dancing.begin(), still_dancing.end(), [](const coord_t& d){ return g_board2[d.first][d.second].well_dancing; });
        if (victims == still_dancing.end()) break;

        for (auto v = victims; v != still_dancing.end(); ++v) {
            const auto [r, c] = *v;
            const auto& d = g_board2[r][c];
            survivors_skills -= d.skill;
            if (c > 0 && d.dist_W > 0) {
                auto& w = g_board2[r][c - d.dist_W];
                if (d.dist_E > 0) {
                    w.E = d.E;
                    w.dist_E += d.dist_E;
                } else
